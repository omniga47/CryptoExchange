...
function LoadAltKraData() {
    ...
    $.ajax({
            ...
            if (data.MessageId == 0) {
				...
				'defaultContent': '',
                    ...
                    rowCallback: function (row, data, index) {
                        ...
                        if (data.MrgBtw100k150k >= highlightVal) {
                            $(row).find('td:eq(6)').css('color', 'red');
                            $(row).find('td:eq(6)').css('font-weight', 'bold');
                            hasHighlighted = true;
                        }
                        if (data.MrgBtw150k200k >= highlightVal) {
                            $(row).find('td:eq(10)').css('color', 'red');
                            $(row).find('td:eq(10)').css('font-weight', 'bold');
                            hasHighlighted = true;
                        }
                        if (data.MrgGrt200k >= highlightVal) {
                            $(row).find('td:eq(14)').css('color', 'red');
                            $(row).find('td:eq(14)').css('font-weight', 'bold');
                            hasHighlighted = true;
                        }
		...
		var hours = now.getHours();
                ...
                var inres = hours  + ":" + min + ":" + sec + " " + day + "/" + mon + "/" + now.getFullYear();
